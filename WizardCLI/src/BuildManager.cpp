...
void BuildManager::InitDirectoryNames() {
	//----------[ IMG ]----------//
	#ifdef WIN32
		bool using_texdb = file_exists("\\texdb\\gta3.img");
	#else
		bool using_texdb = file_exists("/texdb/gta3.img");
	#endif

	if(using_texdb) {
		ImageFileNames.push_back("/texdb/gta_int.img");
		ImageFileNames.push_back("/texdb/gta3.img");
		if(UsingSAMPObjects) ImageFileNames.push_back("/texdb/SAMPCOL.img");
		if(UsingCustomObjects) ImageFileNames.push_back("/texdb/custom.img");
	}
	else {
		ImageFileNames.push_back("/models/gta_int.img");
		ImageFileNames.push_back("/models/gta3.img");
		if(UsingSAMPObjects) ImageFileNames.push_back("/models/SAMPCOL.img");
		if(UsingCustomObjects) ImageFileNames.push_back("/models/custom.img");
	}

	//----------[ COL ]----------//
	CollisionFileNames.push_back("/models/coll/weapons.col");
	CollisionFileNames.push_back("/models/coll/vehicles.col");
	//CollisionFileNames.push_back("/models/coll/peds.col"); //Unusual file structure
...
bool BuildManager::ExtractImageFiles() {
	for(std::string& name : ImageFileNames) {
		reporter->addFormattedMessage("\tOpening archive: %s", name.c_str());
		
		ImgFile thisFile(GTAInstallDirectory + name);
		
		if(thisFile.Load()) {
			for(ImgEntry& e : thisFile.COL) {
				reporter->addFormattedMessage("\t\tLoading: %s", e.name);
			...
			reporter->addMessage("\t\tDone!\n");
		}
		else {
			reporter->addMessage("\t\tERROR: Failed to open file!\n");
		}
	}
...
bool BuildManager::ExtractCollisionFiles() {
	for(std::string& col : CollisionFileNames) {
		reporter->addFormattedMessage("\tLoading: %s", col.c_str());
	...
	if(IPLFile.is_open()) {
		...
		if(strcmp(IPLHeader, "bnry") != 0) {
			...
			while(std::getline(IPLFile, thisLine)) {
				...
				thisLine.erase(
					std::find_if(thisLine.rbegin(), thisLine.rend(),
				...
				if(thisLine == "inst") {
					...
					while(std::getline(paramStream, thisParam, ',')) {
						thisParam.erase(thisParam.begin(),
							std::find_if(thisParam.begin(), thisParam.end(),
								[](unsigned char c) { return !std::isspace(c); }
							)
						);

						thisParam.erase(
							std::find_if(thisParam.rbegin(), thisParam.rend(),
								[](unsigned char c) { return !std::isspace(c); }
							).base(),
							thisParam.end()
						);
...
bool BuildManager::ExtractItemPlacementFiles() {
	...
	for(std::string& ipl : ItemPlacementFileNames) {
		if(ReadItemPlacementData((GTAInstallDirectory + ipl).c_str())) {
			reporter->addFormattedMessage("\tLoaded: %s", ipl.c_str());
		}
		else {
			reporter->addFormattedMessage("\tFailed to load: %s", ipl.c_str());
			ReturnValue = false;
		}
	}
...
bool BuildManager::ExtractItemDefinitionFiles() {
	...
	for(std::string& ide : ItemDefinitionFileNames) {
		if(ReadItemDefinitionFile((GTAInstallDirectory + ide).c_str())) {
			reporter->addFormattedMessage("\tLoaded: %s", ide.c_str());
		}
		else {
			reporter->addFormattedMessage("\tFailed to load: %s", ide.c_str());
			ReturnValue = false;
		}
	}
	...
	if(IDEFile.is_open()) {
		...
		while(std::getline(IDEFile, thisLine)) {
			...
			if(thisLine == "end") {
				...
				if(!thisObject.ModelName.length()) reporter->addFormattedMessage("\tERROR: Object %d has empty name.", thisObject.modelid);
	...
	for(auto it = COLArray.begin(); it != COLArray.end(); ) {
		...
		if(model_it == modelidLookup.end() || model_it->second != it->name) {
			...
			if(modelnameLookup.find(it->name, name_it)) {
				...
				reporter->addFormattedMessage("\tWARN: Collision %s is unused, ignoring.", it->name.c_str());
...
bool BuildManager::WriteBinaryFile(const char fname[]) {
	...
	if(ColAndreasBinaryfile.is_open()) {
		...
		reporter->addFormattedMessage("\tModels: %u", colCount);
		
		uint32_t iplCount = IPLArray.size();
		ColAndreasBinaryfile.write((char*)&iplCount, sizeof(iplCount));
		reporter->addFormattedMessage("\tItem entries: %u", iplCount);
		...
		reporter->addFormattedMessage("\tFile size: %.2f MiB", float(ColAndreasBinaryfile.tellp()) / (1024.0f * 1024.0f));
...
bool BuildManager::file_exists(const std::string& path) {
